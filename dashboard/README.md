...
실제 SECOM 데이터(`data/processed/secom_spotfire_master.csv`)로 대시보드를 로컬 실행한 전체 화면입니다
(배너 + KPI + 필터 + 6개 차트 + 상세 테이블 전부 포함, 잘림 없음).

![SECOM Yield Prediction Analytics Dashboard - 전체 화면](screenshots/dashboard_overview.png)
